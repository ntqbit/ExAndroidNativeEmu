...
class TestThread(unittest.TestCase):
    def __init__(self, methodName):
        ...
        self._is32_before_call = False
        self._is32_after_call = False

        self._is64_before_call = False
        self._is64_after_call = False
    ...
    def test_thread32(self):
        ...
        h.fun_hook(sym, 4, self._pthread_create32_before_hook,
                   self._pthread_create32_after_hook)
        libdemo = emulator.load_library("tests/bin/libdemo.so")
        r = emulator.call_symbol(libdemo, "test_thread", 3)
        self.assertEqual(r, 3)
        self.assertTrue(self._is32_before_call)
        self.assertTrue(self._is32_after_call)
    ...
    def test_thread64(self):
        ...
        #print("sym : %s"%hex(sym))
        h = FuncHooker(emulator)
        h.fun_hook(sym, 4, self._pthread_create64_before_hook,
                   self._pthread_create64_after_hook)
        #emulator.mu.hook_add(UC_HOOK_CODE, hook_code, emulator)
        libdemo = emulator.load_library("tests/bin64/libdemo.so")
        r = emulator.call_symbol(libdemo, "test_thread", 3)
        self.assertEqual(r, 3)

        self.assertTrue(self._is64_before_call)
        self.assertTrue(self._is64_after_call)
