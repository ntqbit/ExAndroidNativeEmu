...
class SymbolHooks:
        ...
        if not emu.get_muti_task_support():
            modules.add_symbol_hook(
                ...
                hooker.write_function(
                    self.pthread_create))
            modules.add_symbol_hook(
                'pthread_join', hooker.write_function(
                    self.pthread_join))
            modules.add_symbol_hook(
                'pthread_detach',
                hooker.write_function(
    ...
    def system_property_get(self, uc, name_ptr, buf_ptr):
        ...
        logger.debug(
            "Called __system_property_get(%s, 0x%x)" %
            (name, buf_ptr))
        ...
        for mod in self._modules.modules:
            if mod.base <= addr < mod.base + mod.size:
                ...
                memory_helpers.write_ptrs_sz(
                    uc, info_ptr, [
                        dli_fname, mod.base, 0, 0], self._emu.get_ptr_size())
                logger.debug(
    ...
    def dlsym(self, uc, handle, symbol):
        ...
        logger.debug(
            "Called dlsym(0x%x, %s) return 0x%08X" %
            (handle, symbol_str, r))
        return r
