...
class VirtualFileSystem:
    def __init__(self, emu, root_path, cfg, syscall_handler, memory_map):
        ...
        if self._emu.get_arch() == Arch.ARM32:
            ...
            syscall_handler.set_handler(0x27, "mkdir", 2, self._mkdir)
            ...
            syscall_handler.set_handler(0xB4, "pread64", 4, self._handle_pread64)
            ...
            syscall_handler.set_handler(0x10B, "fstatfs64", 3, self._fstatfs64)
            ...
            syscall_handler.set_handler(0x2B, "statfs", 3, self._statfs64)
            syscall_handler.set_handler(0x38, "openat", 4, self._handle_openat)
            syscall_handler.set_handler(0x22, "mkdirat", 3, self._mkdirat)
            # no fstatat64
    ...
    def translate_path(self, path):
        return misc_utils.vfs_path_to_system_path(self._root_path, path)
        ...
        if filename == "/dev/urandom":
            ...
            if filename2 == map_path:
                with open(file_path, "w") as f:
                    self._memory_map.dump_maps(f)

            cmdline_path = "/proc/self/cmdline"
            if filename2 == cmdline_path:
                with open(file_path, "w") as f:
                    # TODO put to config
                    content = self._emu.environment.get_package_name()
                    f.write(content)
            ...
            if filename2 == status_path:
                with open(file_path, "w") as f:
                    # TODO put to config
                    name = self._emu.environment.get_package_name()
    ...
    def _get_config_uid(self, filename_in_vm):
        ...
        if filename_norm in self._root_list:
            ...
            uid = self._emu.environment.get_uid()
        return uid
    ...
    def _handle_read(self, mu, fd, buf_addr, count):
        ...
        logger.debug("Reading %d bytes from '%s'", count, file.get_name())
    ...
    def _handle_close(self, mu, fd):
        ...
        """
        try:
            if self._pcb.has_fd(fd):
                self._pcb.remove_fd(fd)
                os.close(fd)
                self._del_fd_link(fd)
            else:
                logger.warning("fd 0x%08X not in fds maybe has closed, just return 0", fd)
                return 0
        except OSError:
    ...
    def _handle_access(self, mu, filename_ptr, flags):
        filename = memory_helpers.read_utf8(mu, filename_ptr)
        vfs_path = self.translate_path(filename)
    ...
    def _handle_pread64(self, mu, fd, buf, count, pos):
        file = self._pcb.get_fd_detail(fd)
        logger.debug("Reading %d bytes from '%s' at position %d", count, file.get_name(), pos)

        os.lseek(fd, pos, os.SEEK_SET)
        data = os.read(fd, count)

        logger.debug("read return %s", data.hex())
        result = len(data)
    ...
    def _handle_fstat64(self, mu, fd, stat_ptr):
        ...
        stats = os.fstat(fd)
        uid = self._get_config_uid(detail.get_name())
        st_mode = self._fix_st_mode(detail.get_name(), stats.st_mode)
    ...
    def _write_statv_to_buf(self, mu, statv, buf):
        """
    ...
    def _statfs64(self, mu, path_ptr, sz, buf):
        ...
        self._write_statv_to_buf(mu, statv, buf)
        return 0

    def _fstatfs64(self, mu, fd, sz, buf):
        if not self._pcb.has_fd(fd):
            return -1

        fd_detail = self._pcb.get_fd_detail(fd)
        name_in_system = fd_detail.get_name_in_system()
        statv = os.statvfs(name_in_system)
        self._write_statv_to_buf(mu, statv, buf)
        return 0
    ...
    def _handle_openat(self, mu, dfd, filename_ptr, flags, mode):
        ...
        """
        filename = memory_helpers.read_utf8(mu, filename_ptr)
        filepath = self._dirfd_2_path(dfd, filename)
        if filepath is None:
            return -1
    ...
    def _handle_fstatat64(self, mu, dirfd, pathname_ptr, buf, flags):
        ...
        pathname = self.translate_path(pathname_vm)
        ...
        uid = self._get_config_uid(pathname_vm)
